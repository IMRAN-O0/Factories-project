...
import { Plus, Trash2, Save, Download, CheckCircle, XCircle, AlertTriangle, Award, Eye, Thermometer } from 'lucide-react';
...
const FinalProductInspectionForm = () => {
  ...
  const physicalTestNames = ['الوزن', 'الطول', 'العرض', 'السماكة', 'الكثافة', 'اللزوجة', 'درجة الصلابة', 'نقطة الانصهار'];
  const chemicalTestNames = ['الرطوبة', 'الحموضة', 'الدهون', 'البروتين', 'الكربوهيدرات', 'الملح', 'السكر', 'الرماد'];
  const microTestNames = ['العد الكلي للبكتيريا', 'الخمائر والعفن', 'البكتيريا المرضية', 'السالمونيلا', 'الإشريكية القولونية', 'المكورات العنقودية'];
  const sensoryTestNames = ['المظهر العام', 'اللون', 'الرائحة', 'الطعم', 'القوام', 'الملمس'];
  const packagingTestNames = ['سلامة التعبئة', 'إحكام الغلق', 'البيانات التوضيحية', 'تاريخ الصلاحية', 'وزن المحتوى', 'شكل العبوة الخارجي'];
  ...
  const getResultColor = (result) => {
    switch (result) {
      case 'مطابق': return 'text-green-700 bg-green-100 border-green-300';
      case 'غير مطابق': return 'text-red-700 bg-red-100 border-red-300';
      case 'حدي': return 'text-yellow-700 bg-yellow-100 border-yellow-300';
      case 'مقبول': return 'text-green-700 bg-green-100 border-green-300';
      case 'مرفوض': return 'text-red-700 bg-red-100 border-red-300';
      case 'مقبول مع تحفظ': return 'text-yellow-700 bg-yellow-100 border-yellow-300';
  ...
  const getResultIcon = (result) => {
    switch (result) {
      case 'مطابق':
      case 'مقبول': return CheckCircle;
      case 'غير مطابق':
      case 'مرفوض': return XCircle;
      case 'حدي':
      case 'مقبول مع تحفظ': return AlertTriangle;
            ...
            <table className="w-full min-w-max">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">اسم الفحص</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">المواصفة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">القيمة الفعلية</th>
                    ...
                    <tr key={test.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          ...
                          <ResultIcon size={16} className={test.result === 'مطابق' ? 'text-green-600' : test.result === 'غير مطابق' ? 'text-red-600' : 'text-yellow-600'} />
                        </div>
                      </td>
                      <td className="px-3 py-2">
                        <input
                    ...
                    <tr key={test.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">
                        <input
                          type="text"
                          value={test.specification}
                          onChange={(e) => handleTestChange('microbiologicalTests', index, 'specification', e.target.value)}
                          className="w-full border border-gray-300 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                          placeholder="< 1000"
                        />
                      </td>
            ...
            <table className="w-full min-w-max">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">اسم الفحص</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">المواصفة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">الوصف الفعلي</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">النتيجة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">ملاحظات</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">حذف</th>
                </tr>
