...
import { FileText, Download, Save, Plus, Trash2, Calendar, User, Package, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
  ...
  const saveRecord = () => {
    try {
      ...
      if (emptyItems.length > 0) {
        alert(isRTL ? 'يرجى ملء جميع معايير الفحص أو حذف الصفوف الفارغة' : 'Please fill all inspection parameters or remove empty rows');
        return;
      }
      ...
      setInspectionItems([{ parameter: '', specification: '', result: '', status: 'pending', method: '', remarks: '' }]);
    ...
    return `
      ...
      <body>
          ...
          ${inspectionItems.map(item => `
            <tr>
              ...
              <td class="status-${item.status}">
                ${item.status === 'passed' ? (isRTL ? 'مقبول' : 'Passed') : 
                  item.status === 'failed' ? (isRTL ? 'مرفوض' : 'Failed') : 
        ...
        <div class="overall-status status-${status}">
          ...
          ${status === 'passed' ? (isRTL ? 'مقبول' : 'APPROVED') : 
            status === 'failed' ? (isRTL ? 'مرفوض' : 'REJECTED') : 
            (isRTL ? 'قيد الانتظار' : 'PENDING')}
        </div>
            ...
            <div className="flex items-center space-x-4 rtl:space-x-reverse">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isRTL ? 'نظام ضمان الجودة' : 'Quality Assurance System'}
                </p>
              </div>
            </div>
            ...
            <div className="mb-8">
                      ...
                      <tr key={index} className="border-b">
                        ...
                        <td className="px-4 py-3">
                          ...
                          >
                            ...
                            <option value="microbiological">{isRTL ? 'فحص ميكروبيولوجي' : 'Microbiological Test'}</option>
                          </select>
                        </td>
              ...
              <div className="mt-4 p-4 rounded-lg border">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`flex items-center px-3 py-2 rounded-full text-sm font-medium ${getStatusColor(status)}`}>
                    {getStatusIcon(status)}
                    <span className="mr-2">
                      {status === 'passed' ? (isRTL ? 'جميع المعايير مقبولة' : 'All Parameters Passed') : 
                       status === 'failed' ? (isRTL ? 'يوجد معايير مرفوضة' : 'Some Parameters Failed') : 
                       (isRTL ? 'قيد المراجعة' : 'Under Review')}
                    </span>
                  </div>
                </div>
