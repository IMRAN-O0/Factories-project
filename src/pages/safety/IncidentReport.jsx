...
const IncidentReport = () => {
  ...
  const incidentTypes = ['إصابة عمل', 'يكاد أن يحدث', 'ضرر في الممتلكات', 'انسكاب مواد', 'حريق', 'انقطاع كهرباء', 'عطل معدات', 'حادث مروري', 'أخرى'];
  ...
  const bodyParts = ['الرأس', 'العين', 'الوجه', 'الرقبة', 'الكتف', 'الذراع', 'اليد', 'الأصابع', 'الصدر', 'الظهر', 'البطن', 'الساق', 'القدم', 'أخرى'];
  ...
  const getSeverityColor = (severity) => {
    switch (severity) {
      case 'طفيف': return 'text-green-700 bg-green-100 border-green-300';
      case 'متوسط': return 'text-yellow-700 bg-yellow-100 border-yellow-300';
      case 'شديد': return 'text-orange-700 bg-orange-100 border-orange-300';
      case 'خطير جداً': return 'text-red-700 bg-red-100 border-red-300';
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'مفتوح': return 'text-red-700 bg-red-100 border-red-300';
      case 'تحت التحقيق': return 'text-yellow-700 bg-yellow-100 border-yellow-300';
      case 'تحت المعالجة': return 'text-blue-700 bg-blue-100 border-blue-300';
      case 'مكتمل': return 'text-green-700 bg-green-100 border-green-300';
      case 'مغلق': return 'text-gray-700 bg-gray-100 border-gray-300';
      case 'مخطط': return 'text-blue-700 bg-blue-100 border-blue-300';
      case 'في التنفيذ': return 'text-yellow-700 bg-yellow-100 border-yellow-300';
      case 'متأخر': return 'text-red-700 bg-red-100 border-red-300';
      case 'ملغي': return 'text-gray-700 bg-gray-200 border-gray-400';
      default: return 'text-gray-700 bg-gray-100 border-gray-300';
    }
  };
  ...
  const calculateSummary = () => {
    ...
    const estimatedTotalCost = formData.damages.reduce((sum, damage) => sum + (parseFloat(damage.estimatedCost) || 0), 0);
    ...
    return (
      <div className="print-container">
        ...
        <div className="border border-gray-300 p-4">
          <p><strong>رقم التقرير:</strong> {formData.incidentNumber}</p>
          <p><strong>تاريخ التقرير:</strong> {formData.reportDate}</p>
          <p><strong>تاريخ الحادث:</strong> {formData.incidentDate}</p>
          <p><strong>وقت الحادث:</strong> {formData.incidentTime}</p>
          <p><strong>نوع الحادث:</strong> {formData.incidentType}</p>
          <p><strong>مستوى الخطورة:</strong> {formData.severity}</p>
          <p><strong>وصف الحادث:</strong> {formData.description}</p>
          <p><strong>الإجراءات الفورية:</strong> {formData.immediateActions}</p>
          <p><strong>السبب الجذري:</strong> {formData.rootCause}</p>
        </div>
      </div>
    );
  }
            ...
            <table className="w-full min-w-max">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">اسم المصاب</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">المنصب</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">القسم</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">نوع الإصابة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">العضو المصاب</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">الخطورة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">العلاج</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">حذف</th>
                </tr>
              </thead>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-max">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">#</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">نوع العنصر</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">اسم العنصر</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">وصف الضرر</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">التكلفة المقدرة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">حالة الإصلاح</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">حذف</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-max">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">#</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">اسم الشاهد</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">المنصب</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">القسم</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">معلومات الاتصال</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">الشهادة</th>
                  <th className="px-3 py-2 text-right text-xs font-medium text-gray-700 uppercase tracking-wider">حذف</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
