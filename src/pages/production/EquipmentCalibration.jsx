...
import { Gauge, Download, Save, Plus, Trash2, Calendar, User, Target, CheckCircle, XCircle, AlertTriangle, TrendingUp } from 'lucide-react';
...
const EquipmentCalibrationForm = () => {
  ...
  const handlePointChange = (index, field, value) => {
    ...
    if ((field === 'referenceValue' || field === 'actualValue') && 
        updatedPoints[index].referenceValue && updatedPoints[index].actualValue) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'passed': return <CheckCircle className="text-green-600" size={16} />;
  ...
  const saveRecord = () => {
    try {
      ...
      if (emptyPoints.length > 0) {
        alert(isRTL ? 'يرجى ملء جميع نقاط المعايرة أو حذف الصفوف الفارغة' : 'Please fill all calibration points or remove empty rows');
        return;
      }
      ...
      setCalibrationPoints([{ parameter: '', referenceValue: '', actualValue: '', deviation: '', tolerance: '', status: 'pending', adjustmentMade: '', finalValue: '', notes: '' }]);
          ...
          ${calibrationPoints.map((point, index) => `
            <tr>
              ...
              <td class="status-${point.status}">
                ${point.status === 'passed' ? (isRTL ? 'نجح' : 'Passed') : 
                  point.status === 'warning' ? (isRTL ? 'تحذير' : 'Warning') : 
                  point.status === 'failed' ? (isRTL ? 'فشل' : 'Failed') :
          ...
          <div class="cert-box">
            ...
            <div style="font-size: 14px; margin-bottom: 10px;">
              ...
              <span style="padding: 5px 10px; border-radius: 15px; margin-left: 10px; background: ${formData.overallStatus === 'passed' ? '#dcfce7' : formData.overallStatus === 'adjusted' ? '#fef3c7' : '#fecaca'};">
                ${formData.overallStatus === 'passed' ? (isRTL ? 'نجحت - الجهاز دقيق' : 'PASSED - Equipment Accurate') : 
                  formData.overallStatus === 'adjusted' ? (isRTL ? 'تم التعديل - أصبح دقيق' : 'ADJUSTED - Now Accurate') : 
                  formData.overallStatus === 'failed' ? (isRTL ? 'فشلت - الجهاز غير دقيق' : 'FAILED - Equipment Inaccurate') :
                  (isRTL ? 'استخدام محدود' : 'LIMITED USE')}
              </span>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 rtl:space-x-reverse">
              {tenantConfig.logo && (
                <img src={tenantConfig.logo} alt="Logo" className="h-12 w-auto" />
              )}
              <div>
            ...
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
              <div className={`px-3 py-1 rounded-full text-sm font-medium border ${
                formData.overallStatus === 'passed' ? 'bg-green-100 text-green-800 border-green-200' :
                formData.overallStatus === 'adjusted' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                formData.overallStatus === 'failed' ? 'bg-red-100 text-red-800 border-red-200' :
                'bg-gray-100 text-gray-800 border-gray-200'
              }`}>
                {formData.overallStatus === 'passed' ? '✓' : 
                 formData.overallStatus === 'adjusted' ? '⚙️' : 
                 formData.overallStatus === 'failed' ? '✗' : '⚠️'}
                <span className="mr-1">
                  {formData.overallStatus === 'passed' ? (isRTL ? 'دقيق' : 'Accurate') : 
                   formData.overallStatus === 'adjusted' ? (isRTL ? 'معدل' : 'Adjusted') : 
                   formData.overallStatus === 'failed' ? (isRTL ? 'فاشل' : 'Failed') : 
                   (isRTL ? 'محدود' : 'Limited')}
                </span>
              </div>
              {stats.totalPoints > 0 && (
                <div className={`px-3 py-1 rounded-full text-sm font-medium border ${
                  stats.passRate >= 90 ? 'bg-green-100 text-green-800 border-green-200' :
                  stats.passRate >= 70 ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
          ...
          <div className="border-b border-gray-200 p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-gray-500">
                {isRTL ? `تاريخ اليوم: ${new Date().toLocaleDateString('ar-SA')}` : `Today: ${new Date().toLocaleDateString('en-US')}`}
              </div>
            </div>
          </div>
      ...
      <div className="bg-white border-t mt-8">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center text-sm text-gray-600">
            ...
            <div>
              {isRTL ? `تم الإنشاء في: ${new Date().toLocaleDateString('ar-SA')}` : `Created on: ${new Date().toLocaleDateString('en-US')}`}
            </div>
          </div>
        </div>
      </div>
