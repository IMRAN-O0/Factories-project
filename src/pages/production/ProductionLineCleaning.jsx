...
import { Droplets, Download, Save, Plus, Trash2, Calendar, User, Beaker, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
const ProductionLineCleaningForm = () => {
  ...
  const addStep = () => {
    setCleaningSteps([...cleaningSteps, { 
      step: '', 
      area: '', 
      method: '',
      cleaningAgent: '',
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'in_progress': return <AlertTriangle className="text-blue-600" size={16} />;
  ...
  const saveRecord = () => {
    try {
      ...
      if (emptySteps.length > 0) {
        alert(isRTL ? 'يرجى ملء جميع خطوات التنظيف أو حذف الصفوف الفارغة' : 'Please fill all cleaning steps or remove empty rows');
        return;
      }
      ...
      setCleaningSteps([{ step: '', area: '', method: '', cleaningAgent: '', concentration: '', contactTime: '', temperature: '', responsible: '', status: 'pending', verification: '', notes: '' }]);
        ...
        <div class="summary-box">
          <div>
            ...
            <span class="status-${formData.overallStatus === 'approved' ? 'completed' : formData.overallStatus === 'rejected' ? 'failed' : 'pending'}" style="padding: 4px 8px; border-radius: 10px; margin-left: 10px;">
              ${formData.overallStatus === 'approved' ? (isRTL ? 'معتمد للإنتاج' : 'Approved') : 
                formData.overallStatus === 'rejected' ? (isRTL ? 'مرفوض' : 'Rejected') : 
                (isRTL ? 'معتمد بشروط' : 'Conditional')}
            </span>
          </div>
          <div>
          ...
          ${cleaningSteps.map((step, index) => `
            <tr>
              ...
              <td class="status-${step.status}">
                ${step.status === 'completed' ? (isRTL ? 'مكتملة' : 'Completed') : 
                  step.status === 'in_progress' ? (isRTL ? 'قيد التنفيذ' : 'In Progress') : 
                  step.status === 'failed' ? (isRTL ? 'فشلت' : 'Failed') :
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
              <div className={`px-3 py-1 rounded-full text-sm font-medium border ${
                formData.overallStatus === 'approved' ? 'bg-green-100 text-green-800 border-green-200' :
                formData.overallStatus === 'rejected' ? 'bg-red-100 text-red-800 border-red-200' :
                'bg-yellow-100 text-yellow-800 border-yellow-200'
              }`}>
                {formData.overallStatus === 'approved' ? '✓' : 
                 formData.overallStatus === 'rejected' ? '✗' : '⚠️'}
                <span className="mr-1">
                  {formData.overallStatus === 'approved' ? (isRTL ? 'معتمد' : 'Approved') : 
                   formData.overallStatus === 'rejected' ? (isRTL ? 'مرفوض' : 'Rejected') : 
                   (isRTL ? 'بشروط' : 'Conditional')}
                </span>
              </div>
              {completionPercentage > 0 && (
                <div className={`px-3 py-1 rounded-full text-sm font-medium border ${
                  completionPercentage === 100 ? 'bg-green-100 text-green-800 border-green-200' :
                  completionPercentage >= 75 ? 'bg-blue-100 text-blue-800 border-blue-200' :
                  'bg-yellow-100 text-yellow-800 border-yellow-200'
                }`}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="font-medium text-blue-800">
                    {isRTL ? 'ملخص التنظيف:' : 'Cleaning Summary:'}
                  </span>
                </div>
