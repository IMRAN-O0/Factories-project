...
const FormRenderer = () => {
  ...
  const generatePDF = () => {
    try {
      ...
      alert(isRTL ? 'حدث خطأ في تصدير PDF. جرب النسخ والطباعة يدوياً' : 'Error generating PDF. Try manual copy and print');
    }
  };
  ...
  const saveRecord = () => {
    try {
      ...
      if (emptyMaterials.length > 0) {
        alert(isRTL ? 'يرجى ملء جميع بيانات المواد أو حذف الصفوف الفارغة' : 'Please fill all material data or remove empty rows');
        return;
      }

      // Check quantity balance
      if (!quantitiesMatch) {
        const confirmSave = confirm(isRTL ? 
            ...
            <div className="flex items-center space-x-4 rtl:space-x-reverse">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isRTL ? 'نظام إدارة الإنتاج' : 'Production Management System'}
                </p>
              </div>
            </div>
