...
import { Settings, Download, Save, Plus, Trash2, Calendar, User, Wrench, AlertCircle, CheckCircle, Clock } from 'lucide-react';
  ...
  const saveRecord = () => {
    try {
      ...
      if (emptyTasks.length > 0) {
        alert(isRTL ? 'يرجى ملء جميع مهام الصيانة أو حذف الصفوف الفارغة' : 'Please fill all maintenance tasks or remove empty rows');
        return;
      }
      ...
      setMaintenanceTasks([{ task: '', description: '', status: 'pending', startTime: '', endTime: '', technician: '', spareParts: '', notes: '' }]);
          ...
          ${maintenanceTasks.map(task => `
            <tr>
              ...
              <td class="status-${task.status}">
                ${task.status === 'completed' ? (isRTL ? 'مكتملة' : 'Completed') : 
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 rtl:space-x-reverse">
              <div className={`px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(formData.overallStatus || 'pending')}`}>
                {getStatusIcon(formData.overallStatus || 'pending')}
                <span className="mr-1">
                  {formData.overallStatus === 'completed' ? (isRTL ? 'مكتملة' : 'Completed') : 
                   formData.overallStatus === 'in_progress' ? (isRTL ? 'قيد التنفيذ' : 'In Progress') : 
                   (isRTL ? 'في الانتظار' : 'Pending')}
                </span>
              </div>
              {formData.priority && (
                <div className={`px-3 py-1 rounded-full text-sm font-medium border ${getPriorityColor(formData.priority)}`}>
                  {isRTL ? 
                    (formData.priority === 'critical' ? 'حرج' : 
                     formData.priority === 'high' ? 'عالي' : 
                     formData.priority === 'medium' ? 'متوسط' : 'منخفض') :
